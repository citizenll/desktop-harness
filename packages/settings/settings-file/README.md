...
- **Write-back is atomic, owner-only, and symlink-proof.** The render exclusive-creates a random-suffix temp sibling with mode `0600` (`wx` refuses to follow a planted symlink) and renames over the target, cleaning the temp up on failure. On Windows, transient `EACCES`/`EPERM`/`EBUSY` replacement contention retries for at most 2 seconds without deleting the target or abandoning atomic publication.
