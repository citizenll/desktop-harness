...
Node 侧会扫描已启用的 Loader 配置项以发现 GUI `dsh.client` 包，解析每个 `exports["./client"]`，把构建后的 bundle 哈希写入启动图，并以独立于载体的方式暴露图与 `clientPath(id)`。存在 `ctx.webServer` 时，它会在 `/plugins` 下挂载 bundle 与 sourcemap，并把启动清单注入浏览器 index；Electron 则通过 `dsh://app` 读取同一个注册表。源码启动会把宿主侧导入映射到 TypeScript 源码，但仍消费这一构建后的客户端导出；缺失文件共享一条构建说明，随后以包／路径列表列出各项，而无关的文件系统错误仍是独立故障。
