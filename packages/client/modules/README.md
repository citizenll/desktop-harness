...
The Node half scans enabled Loader entries for GUI `dsh.client` packages, resolves each `exports["./client"]`, hashes the built bundle into the boot graph, and exposes the graph plus `clientPath(id)` independently of a carrier. When `ctx.webServer` is present it mounts the bundles and source maps under `/plugins` and injects the boot manifest into the browser index; Electron reads the same registry through `dsh://app`. Source launch maps host imports to TypeScript source but still consumes this built client export; missing files share one build instruction followed by a package/path list, while unrelated filesystem errors remain separate failures.
