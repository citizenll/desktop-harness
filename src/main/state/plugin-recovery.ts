...
export async function resetPluginProfile(
  ...
  try {
    ...
    // Desktop now composes dsh-market from the immutable app runtime. Retire
    // any legacy profile registration even when recovery targets a different
    // plugin, otherwise the next boot can recreate the duplicate owner.
    if (manifest.dependencies?.dshmarket) {
      delete manifest.dependencies.dshmarket
      modified = true
    }
    if (manifest.dsh?.profile?.bundles?.includes('dshmarket')) {
      manifest.dsh.profile.bundles = manifest.dsh.profile.bundles.filter(
        (bundle) => bundle !== 'dshmarket'
      )
      modified = true
    }
    ...
    if (failingPlugin) {
      ...
      const safeBundleSet = new Set(safeBundles)
      manifest.dsh ??= {}
    ...
    if (existsSync(nodeModulesPath)) {
      const legacyMarketDirectory = join(nodeModulesPath, 'dshmarket')
      if (existsSync(legacyMarketDirectory)) {
        await rm(legacyMarketDirectory, { recursive: true, force: true }).catch(() => undefined)
        modified = true
      }
      if (failingPlugin) {
