...
const CAPTION_SAFE_ATTRIBUTE = 'data-dsh-windows-caption-safe'
const CAPTION_BASE_PADDING_PROPERTY = '--dsh-windows-caption-base-padding'
const CAPTION_OVERLAP_PROPERTY = '--dsh-windows-caption-overlap'
...
export function mountWindowsTitlebar(options: TitlebarMountOptions): void {
  ...
  installCaptionAreaProtection(document, menuButton)
...
function installLayout(document: Document): void {
  ...
  style.textContent = `
    ...
    body.dsh-desktop-windows-titlebar-layout [${CAPTION_SAFE_ATTRIBUTE}] {
      padding-right: calc(
        var(${CAPTION_BASE_PADDING_PROPERTY}, 0px) +
        var(${CAPTION_OVERLAP_PROPERTY}, 0px)
      ) !important;
    }
  `
...
function installCaptionAreaProtection(document: Document, menuButton: HTMLButtonElement): void {
  const sync = (): void => {
    const exclusionLeft = menuButton.getBoundingClientRect().left
    if (exclusionLeft <= 0) return

    const utilityContainers = document.querySelectorAll<HTMLElement>(
      '[class*="_headerUtilities"]'
    )
    const headers = new Set<HTMLElement>()
    for (const utilities of utilityContainers) {
      const header = utilities.closest<HTMLElement>('header')
      if (header) headers.add(header)
    }

    for (const header of headers) {
      resizeObserver.observe(header)
      if (!header.hasAttribute(CAPTION_SAFE_ATTRIBUTE)) {
        const basePadding = document.defaultView?.getComputedStyle(header).paddingRight ?? '0px'
        header.setAttribute(CAPTION_SAFE_ATTRIBUTE, '')
        header.style.setProperty(CAPTION_BASE_PADDING_PROPERTY, basePadding)
      }
      header.style.setProperty(
        CAPTION_OVERLAP_PROPERTY,
        `${captionAreaOverlap(header.getBoundingClientRect(), exclusionLeft)}px`
      )
    }
  }
...
export function captionAreaOverlap(
  header: Pick<DOMRect, 'top' | 'bottom' | 'right'>,
  exclusionLeft: number
): number {
  if (header.bottom <= 0 || header.top >= WINDOWS_TITLEBAR_HEIGHT) return 0
  return Math.max(0, Math.ceil(header.right - exclusionLeft))
}
