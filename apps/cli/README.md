...
The invoking directory is the default workspace root. The `web`, `desktop`, and `headless` profiles auto-initialize on first use from shipped templates; any other profile must be created through `dsh plugin`. The Electron application consumes the `desktop` profile through the exported `@deepseek-ai/dsh/profile-boot` launcher rather than adding a CLI command.
