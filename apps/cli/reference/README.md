...
The `web`, `desktop`, and `headless` profiles auto-initialize from shipped templates on first use (`web`: base + web-app; `desktop`: base + web-app + desktop-app; `headless`: base + headless). Any other missing profile fails loud with a hint to run `dsh plugin --profile <name> add <package>`. The Electron application consumes the desktop template through the exported profile launcher; `dsh --profile desktop` composes the profile but does not create an Electron window.
