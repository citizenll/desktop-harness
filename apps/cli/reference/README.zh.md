...
`web`、`desktop` 和 `headless` profile 首次使用时会从随附模板自动初始化（`web`：base + web-app；`desktop`：base + web-app + desktop-app；`headless`：base + headless）。其他缺失的 profile 会显式报错，并提示运行 `dsh plugin --profile <name> add <package>`。Electron 应用通过导出的 profile 启动器消费 desktop 模板；`dsh --profile desktop` 只会组合该 profile，不会创建 Electron 窗口。
