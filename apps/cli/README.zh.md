...
运行命令时所在的目录将作为默认 workspace 根目录。`web`、`desktop` 和 `headless` profile 在首次使用时会从随附模板自动初始化；其他任何 profile 都必须通过 `dsh plugin` 创建。Electron 应用通过导出的 `@deepseek-ai/dsh/profile-boot` 启动器消费 `desktop` profile，而不是增加新的 CLI 命令。
