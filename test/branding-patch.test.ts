...
describe('DSH Desktop sidebar branding', () => {
  ...
  it('pairs the DSH logo with the original Harness wordmark in the expanded sidebar', async () => {
    const patch = await readFile(
      path.join(projectRoot, 'patches', '@deepseek-ai+dsh-client-ui-sidebar+0.1.1-rc.1.patch'),
      'utf8'
    )
  ...
  it('uses an 80px macOS rail that clears the traffic lights', async () => {
    const patch = await readFile(
      path.join(projectRoot, 'patches', '@deepseek-ai+dsh-client-ui-layout+0.1.1-rc.1.patch'),
      'utf8'
    )
  ...
  it('provides a sidebar phone entry that follows expanded and connected state', async () => {
    ...
    const mobilePanel = await readFile(
      path.join(projectRoot, 'src', 'preload', 'mobile-panel.ts'),
      'utf8'
    )
    const main = await readFile(path.join(projectRoot, 'src', 'main', 'index.ts'), 'utf8')
    ...
    expect(main).toContain("window.webContents.send('mobile:show-panel'")
    expect(main).not.toContain('let mobileWindow: BrowserWindow')
    expect(preload).toContain('installMobilePanel({ document, ipcRenderer, locale })')
    expect(mobilePanel).toContain("ipcRenderer.on('mobile:show-panel'")
    expect(mobilePanel).toContain('width: min(1024px, calc(100vw - 24px))')
    expect(mobilePanel).toContain('border-radius: 12px')
  })
  ...
  it('installs the source logo into the Harness static frontend', async () => {
    ...
    expect(installer).toContain("'build', 'brand.png'")
