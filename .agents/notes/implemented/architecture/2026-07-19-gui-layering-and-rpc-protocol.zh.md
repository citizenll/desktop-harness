...
同时各消费方的物理通道不同（浏览器 HTTP／WebSocket、进程内 fetch／SSE、Electron 自定义协议 fetch／SSE），还需要一个通道无关的消息模型和单一约定真源，让「加一个方法」「换一种载体」互不牵连，且 wire 上的每条消息可类型校验、可观测、可对账。
...
- `apps/` 作为对外导出的应用入口，可以由 Client / Host 混合组装。
    ...
    - `apps/desktop` 在 Electron 内启动同一 profile 与 web client 包，并通过不开放端口的 `dsh://app` 自定义协议提供已构建 shell 与 Host fetch／SSE 接口。
...
| client 插件 | `dsh-client-connection` / `dsh-client-runtime` / `dsh-client-ui-theme` / `dsh-client-i18n` / `dsh-client-ui-layout` / `dsh-client-ui-sidebar` / `dsh-client-ui-conversation` / `dsh-client-ui-trajectory` | renderer 侧 cordis 插件树（wire 消费方、核心服务、主题、i18n、布局、侧栏、对话、轨迹）——见 Web 客户端架构笔记 | 双入口（node 半边=空 apply；实现在 `src/client/`）；消费面唯一经 ApiProxy |
| 应用 | `@deepseek-ai/dsh`（apps/cli）+ `dsh-web-frontend`（apps/web）+ `@deepseek-ai/dsh-desktop`（apps/desktop） | CLI 分发与共享 profile 启动、精简 Vite shell，以及同一 GUI 图之上的 Electron 生命周期／自定义协议 shell | 各应用只加载选中的界面；dist 定位和 Electron 生命周期等知识留在 app |
...
1. **选 fetch 伪造方式**：浏览器同源 HTTP / 进程内 handler 注入 / Electron `dsh://app` 这类具备特权的自定义协议 fetch 载体（见下文「子类表」）。
2. **在 `apps/` 下写拼装模块**：`startHost()` + 客户端子类 + 该应用私有的信号/打印/退出语义；混合体不建包，拼装写在 app 里。
3. **需要 HTTP 承载才 import `dsh-host-webserver`**，否则零端口。

三种应用形态保持这一区分：Web 挂载 Host、HTTP／WebSocket 载体与浏览器组合；Desktop 在同进程自定义协议载体后挂载同一 Host／Client 图，不开放监听端口；`dsh --profile headless` 挂载直接使用核心服务的 runner，不包含 Host、HTTP 或 renderer。ACP 类协议桥不遵循 client 载体清单：它把 core 暴露给外部生态，直接通过 `ctx.plugin(入口插件)` 挂载，不使用 fetch。

## 消息协议

以下各节是前置层（`dsh-host-apiproxy`）承载的协议本体。wire 上只有四种消息（四象限）——右列的 Web 承载只是示例，换载体（进程内／自定义协议）时四象限不变：
...
| `EmbeddedApiClient` | dsh-client-connection | 对 `dsh://app` 使用 `globalThis.fetch`，并以同一自定义协议承载 SSE 下行 | Electron renderer；无需 preload bridge、WebSocket、HTTP 监听器或协议层改动 |
