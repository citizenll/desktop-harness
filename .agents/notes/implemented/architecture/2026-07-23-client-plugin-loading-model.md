...
The shared tsdown preset emits `client.js.map` for every plugin and rewrites first-party source paths into the renderer-resolvable repository shape `/packages/<group>/<package>/src/...`. Other workspace sources inlined into a bundle likewise resolve to their `packages/` owner, while dependency paths remain unchanged; `sourcesContent` carries the source, so the active carrier serves the map at `/plugins/<id>/client.js.map` and exposes no source route. The Vite shell also emits source maps, letting both shell code and out-of-graph plugins map stacks and performance profiles back to TypeScript/TSX.
...
What happens between a GUI profile starting and the UI appearing? Three stages: the Host composes a carrier-independent graph, the Web HTTP adapter or Electron custom protocol serves it, the shell prefetches, then cordis orchestrates.
...
| `dsh-client-connection` | wire layer | plugin (`dsh.client` + bundle), declares `immediately` | Web HTTP/WebSocket and Electron custom-protocol fetch/SSE carriers |
...
Roster: it lives in the web bundle's config tree (`packages/bundle/web-app/cordis.patch.yml`), which the desktop bundle layers and selectively patches; `mountWebPlugins` and the `CLIENT_PACKAGES` constant are gone, and recomposing a deployment means swapping the yml/overlay. The graph composer moved from a webserver-side registry into the `dsh-client-modules` node half (the package upgraded to dual-face per this note's promotion rule — its consumer now reaches it through cordis DI), and the transport split landed alongside: the modules registry and Connection Fetch dispatcher are carrier-independent, their optional Web adapters mount `/plugins`, index injection, `/api/*`, and WebSocket downlinks only when `ctx.webServer` exists, Electron serves the same registries through `dsh://app`, and the Web-development bundle watch + SSE channel remains in the hmr node half.
