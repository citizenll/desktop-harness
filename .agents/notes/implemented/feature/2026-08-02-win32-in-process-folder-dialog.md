...
- Real dialog rendering and selection completion stay Windows release checks; the automated open/abort smoke proves open, abort, and unwind without depending on an interactive desktop.
- The COM vtable slots and GUIDs used are frozen Windows ABI (Vista); a koffi signature mistake or an unbounded native-memory read risks a native access violation, contained to the dialog child process — the host Node process survives and the failure surfaces with its exit code and signal (no fallback tier; see the [chain removal](../simplification/2026-08-04-drop-windows-powershell-picker-fallback.md)). The mocked-koffi ABI pins and the real win32 smoke exist to catch such mistakes before shipping.
- The packaged-binary arm — the packaged executable spawning itself as the dialog entry — is not exercised by the automated suite: source and built-worker tests pin the protocol and decoder, while a Windows release smoke accepts a real folder through the packaged Electron executable.
